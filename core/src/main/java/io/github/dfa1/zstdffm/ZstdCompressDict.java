...
///
/// Building a {@code ZstdCompressDict} pre-processes the dictionary so that each
...
public final class ZstdCompressDict extends NativeObject {
	...
	/// Digests {@code dict} for compression at the given level.
	public ZstdCompressDict(ZstdDictionary dict, int level) {
		super(create(dict, level));
		this.level = level;
	}

	/// Digests {@code dict} for compression at the library default level.
